 ...
 */
var options = {
	valueNames: ['contractId', 'contractName']
};
var search_button = function() {
	...
	$.ajax({
		type: "POST", // GET / POST
		url: "/Nerva/SearchContractMstAction", // 送信先のServlet URL
		...
		while (table.rows[0])
			table.deleteRow(0);
		// 通信が成功した場合に受け取るメッセージ
		var searchContract = data["searchContractDTOList"];
		var errorList = data["errorListDTO"];
		const button = document.getElementById("updateContract");
		...
		if (searchContract != null) {
			for (var i = 0; i < searchContract.length; i++) {
				var contractinfo = searchContract[i];
				createData(table.insertRow(-1), i, contractinfo);
			}
...
var createData = function(row, i, contractinfo) {

	var cell1 = row.insertCell(-1);
	cell1.innerHTML = contractinfo.contractId;
	let $new_element1 = document.createElement('input');
	$new_element1.setAttribute("type", "hidden");
	$new_element1.setAttribute("value", contractinfo.contractId);
	$new_element1.classList.add("contractId");

	var cell2 = row.insertCell(-1);
	cell2.innerHTML = '<input type="text"id="contractName' + i + '" class="updating" value="' + contractinfo.contractName + '" onChange="cssChange(' + i + ')"/>';
	$('input[value=請負契約]').attr('disabled','true');
	let $new_element2 = document.createElement('input');
	$new_element2.setAttribute("type", "hidden");
	$new_element2.setAttribute("value", contractinfo.contractName);
...
let cssChange = (i) => {
const button = document.getElementById("updateContract");
var element = document.getElementById('contractName' + i + '');
	var $getId = $('td .contractId').eq(i);
...
/*
var createResultcontract = function() {
	var table = document.getElementById("contract");
