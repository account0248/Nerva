...
public class LoginActionTest {
	...
	public void allInputSuccessPattern() throws ServletException, IOException {
		...
		// パラメータに入力内容を設定
		httpRequest.setParameter("userId", "admin");
		...
		assertThat(session.getAttribute("userId").toString(), is("admin"));
		assertThat(session.getAttribute("userName").toString(), is("管理者"));
	 ...
	 */
	@Test
	@DisplayName("全項目入力_子会社")
	public void allInputSuccessPattern2() throws ServletException, IOException {
		MockHttpRequest httpRequest = new MockHttpRequest();
		MockHttpResponse httpResponse = new MockHttpResponse();
		...
		assertThat(session.getAttribute("companyId").toString(), is("CP00000003"));
		assertThat(session.getAttribute("adminFlg").toString(), true);
	}
